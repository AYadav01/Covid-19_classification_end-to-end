...
class ResNet(nn.Module):
    ...
    def _makeLayers(self, block, num_residual_blocks, out_channels, stride=1):
        ...
        if (stride != 1) or (self.in_channel != out_channels * block.expansion):
            dim_change = nn.Sequential(nn.Conv2d(self.in_channel, out_channels * block.expansion, kernel_size=1,
                                                 stride=stride, bias=False), nn.BatchNorm2d(out_channels * block.expansion))
