...
class TrainModel:
    ...
    def start_training(self, path_to_images, path_to_masks, transformation=None, val_percent=0.2, lr_rate=1e-4):
        if transformation is None:
            transformations_train = transforms.Compose([transforms.ToTensor()])
            dataset = DataProcessor(imgs_dir=path_to_images, masks_dir=path_to_masks, transformation=transformations_train)
        else:
            dataset = DataProcessor(imgs_dir=path_to_images, masks_dir=path_to_masks, transformation=transformation)
        ...
        print("="*40)
        trainloader = DataLoader(train, batch_size=self.batch, shuffle=True, drop_last=True)
        validloader = DataLoader(val, batch_size=self.batch, shuffle=True, drop_last=True)
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            model.train()
            for i, (images, masks) in tqdm(enumerate(trainloader)):
...
if __name__ == "__main__":
    ...
    train_masks = "path_to_train_masks\\"
    train_obj = TrainModel(img_channel, out_num_class, num_epcohs, batches, path_to_weights)
    """
    Note: validation set is create out of training images
    """
    train_obj.start_training(train_images, train_masks)
